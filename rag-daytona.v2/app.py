...
class PlanStepRequest(BaseModel):
    ...
    language: Optional[str] = Field("te", description="Response/narration language code (e.g., 'te', 'en', 'de')")
...
async def plan_next_step(request: PlanStepRequest):
    ...
    try:
        result = await visual_orchestrator.plan_next_step(
            ...
            language=request.language or "te"
        )
        return result
    except Exception as e:
    ...
    async def event_generator():
        ...
        async def run_query():
            try:
                ...
                # Resolve language code to full name (generic for any language)
                from language_utils import resolve_language as _resolve_lang
                lang = _resolve_lang(request.language or "te").lower()
